...
describe('IAP Keys', () => {
  ...
  describe('Coin Pack Values', () => {
    it('should have correct coin values for each pack', () => {
      expect(COIN_PACK_VALUES[CONSUMABLE_PRODUCTS.COINS_SMALL]).toBe(5000);
      expect(COIN_PACK_VALUES[CONSUMABLE_PRODUCTS.COINS_MEDIUM]).toBe(15000);
      expect(COIN_PACK_VALUES[CONSUMABLE_PRODUCTS.COINS_LARGE]).toBe(40000);
    });
  });
  ...
  describe('Display Prices', () => {
    it('should have display prices for all products', () => {
      ...
      expect(DISPLAY_PRICES[CONSUMABLE_PRODUCTS.COINS_SMALL]).toBe('$1.99');
